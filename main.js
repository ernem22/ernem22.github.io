...
const mockBar = (url) => `<div class="bar"><i></i><i></i><i></i><span>${esc(url || '')}</span></div>`;
...
function mockLedger(m) {
  const nav = (m.nav || []).map((n, i) => i === 0 ? `<b>${esc(n)}</b>` : `<span${i === 3 ? ' class="on"' : ''}>${esc(n)}</span>`).join('');
  const btn = (b) => `<span class="btn ${esc(b.v || '')}">${b.spin ? '<i></i>' : ''}${esc(b.t)}</span>`;
  const tokens = (m.tokens || []).map(([label, color]) => `<span style="--c:${esc(color)}">${esc(label)}</span>`).join('');
  const props = (m.props || []).map((p) => `<b>${esc(p.name)}</b><i>${esc(p.type)}</i><i>${esc(p.def)}</i>`).join('');
...
function mockPulse(m) {
  const tiles = (m.tiles || []).map((t) => t.live
    ? `<div class="live"><small>${esc(t.label)}</small><b><i></i>${esc(t.value)}</b></div>`
    : `<div><small>${esc(t.label)}</small><b>${esc(t.value)}${t.unit ? `<em>${esc(t.unit)}</em>` : ''}</b></div>`).join('');
...
function mockVessel(m) {
  ...
  const items = (m.items || []).map((it) => `<div class="item"><i class="pot ${esc(it.cls || '')}"></i><span>${esc(it.name)}</span><em>${esc(it.price)}</em></div>`).join('');
  const lh = (m.scores || []).map((s) => `<div style="--s:${esc(s.value)}"><b>${esc(s.value)}</b><small>${esc(s.label)}</small></div>`).join('');
  const screen = `<div class="screen">
      ${nav}
...
function mockCode(m) {
  const lines = (m.lines || []).map((ln) =>
    `<span class="n">${esc(ln.n || '')}</span>${(ln.tokens || []).map((t) => t.c ? `<span class="${t.c}">${esc(t.t)}</span>` : esc(t.t)).join('')}`).join('\n');
  const term = (m.term || []).map((t, i) => `<span${i === 1 ? ' class="ok"' : ''}>${esc(t)}</span>`).join('');
  const screen = `<div class="screen">
      <pre>${lines}</pre>
...
function tierNames(tier) {
  return (window.DATA?.tech || []).filter((t) => t.tier === tier).map((t) => t.name);
}

/* Detail bodies — each panel's layout describes its scope in data.js */
function renderBody(p) {  const c = p.detail.content;
  switch (p.detail.layout) {
    case 'info': {
      let n = 0;
      const el = (base) => `class="${base ? `${base} ` : ''}rv" style="--i:${++n + 1}"`;
      const stats = c.stats.map((s) =>
        `<div><dt>${esc(s.label)}</dt><dd>${esc(s.value)}${s.unit ? `<small>${esc(s.unit)}</small>` : ''}</dd></div>`).join('');
      const code = c.code.lines.map((line) =>
        line.map((tok) => tok.t === 'plain' ? esc(tok.v) : `<span class="${tok.t}">${esc(tok.v)}</span>`).join('')).join('\n');
      const expList = c.experience ? c.experience.items.map((e) =>
        `<li><span class="yr${e.current ? ' now' : ''}"><span>${esc(e.period)}</span></span><div><strong>${esc(e.role)}</strong><span>${esc(e.place)}</span><p>${esc(e.desc)}</p></div></li>`).join('') : '';
      const lists = (c.lists || []).map((l) =>
        `<div ${el('block')}><span class="label">${esc(l.label)}</span>
        <ul class="plain">${l.items.map((it) => `<li><span>${esc(it.text)}</span><em>${esc(it.meta)}</em></li>`).join('')}</ul></div>`).join('');
    ...
    case 'projects': {
      const items = c.projects.map((project, i) =>
        `<li class="rv" style="--i:${i + 2}"><button type="button" data-p="${i}"${i === 0 ? ' class="is-current"' : ''}><span class="p-num">${String(i + 1).padStart(2, '0')}</span><span class="p-name">${esc(project.name)}<em>${esc(project.type)}</em></span><span class="p-meta">${esc(project.year)}</span></button></li>`).join('');
      const views = c.projects.map((project, i) =>
    ...
    case 'contact': {
      const socials = c.socials.items.map((s) =>
        `<li><a href="${esc(s.href)}"><span>${esc(s.name)}</span><em>${esc(s.handle)}</em><i>${esc(s.arrow || '↗')}</i></a></li>`).join('');
      const terms = c.availability.terms.map((t) =>
        `<li><span>${esc(t.label)}</span><em>${esc(t.value)}</em></li>`).join('');
      const briefs = c.briefs.items.map((b) =>
        `<a class="press" href="mailto:${esc(c.email.address)}?subject=${encodeURIComponent(b.subject)}"><span>${esc(b.subject)}</span><i aria-hidden="true">→</i></a>`).join('');
...
function renderPanel(p) {
  ...
  const count = p.detail.layout === 'projects' ? ` (${p.detail.content.projects.length})` : '';
...
function applyMeta(meta) {
  ...
  if (meta.themeColor) set('meta[name="theme-color"]', 'content', meta.themeColor);
  if (meta.og) {
  ...
  if (meta.twitterCard) set('meta[name="twitter:card"]', 'content', meta.twitterCard);
}
...
function boot(data) {
  ...
  const ACCENT = Object.fromEntries(panels.map((p) => [p.id, `var(--acc-${p.id.replace(/^p-/, '')})`]));
  let hovered = null;
  ...
  const icon = (id) => {
    ...
    return t ? `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="${t.d}"/></svg>` : '';
  };
  ...
  if (cloud) (() => {
    ...
    function layout() {
      ...
      function pick(k) {
        ...
        const unit = Math.min((w - 40) / (spanX + 1), (h - 44) / (spanY + 1), 150);
...
const BOOT_MIN = matchMedia('(prefers-reduced-motion: reduce)').matches ? 350 : 1150;
let bootDone = false;
...
async function start() {
  ...
  if (!data || !Array.isArray(data.panels)) {
    stage.innerHTML = '<p class="boot-error">Could not read <code>data.js</code> — the file must load and define <code>window.DATA</code>.</p>';
    finishBoot();
    return;
  }
  ...
  const shown = new Promise((r) => setTimeout(r, Math.max(0, BOOT_MIN - (performance.now() - bootStart))));
  await Promise.all([painted, fonts, shown]);
  requestAnimationFrame(finishBoot);
}

start().catch((err) => { console.error(err); finishBoot(); });